...
public:
    BitBuffer(List<uint8_t> bytes) : bytes(bytes) {}

    BitBuffer(std::string path) {
        ReadFromFile(path);
    }
    BitBuffer() {
        bytes.Add(0);
    ...
    bool WriteToFile(std::string path) {
        bool b = File::WriteFileBytes(path, GetBuffer());
        return b;
    }

    void ReadFromFile(std::string path) {
        List<uint8_t> data = File::ReadFileBytes(path);
        bytes = data;
    }
};